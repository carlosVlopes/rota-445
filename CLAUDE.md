...
- `alpine-js` — Apply whenever writing or modifying Alpine.js code. Covers `x-data`, `x-show`, `x-model`, `x-for`, `x-ref`, `Alpine.data()`, `Alpine.store()`, AJAX/fetch patterns, and integration with Laravel/Blade. Use for any reactive frontend behavior, modal management, form submissions without page reload, and client-side state management.
